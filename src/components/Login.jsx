...
const Login = () => {
    ...
    try {
        const res = await authServices.verifyOtp({ username: phoneNumber, otp });
        ...
        <div>
            ...
            <div className='px-10 pt-0 pb-8'>
                {!showOtpField ? (<form onSubmit={handlePhoneSubmit}>
                    ...
                    <div className="mt-8">
                        <button type='submit' className='bg-secondary text-white font-bold w-full h-[46px] rounded rounded-md hover:shadow-[0_5px_15px_0_rgba(0,0,0,0.15)]'> Login with OTP </button>
                    </div>
                </form>) : <div>
                    <OtpInput length={4} onOtpSubmit={onOtpSubmit} phoneNumber={phoneNumber} editNumber={handleEditNumber} />
                </div>}
            </div>
        </div>
    )
